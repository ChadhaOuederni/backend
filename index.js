...
const tempConnection = mysql.createConnection({
  host: process.env.DB_HOST || 'database-1.c86roiwoupva.us-east-1.rds.amazonaws.com',
  user: process.env.DB_USER || 'ChadhaOuederni',
  password: process.env.DB_PASSWORD || 'Azerqsdf123*'
});
...
tempConnection.query(`CREATE DATABASE IF NOT EXISTS ${process.env.DB_NAME || 'mydatabase'}`, (err) => {
  ...
  const db = mysql.createConnection({
      ...
      user: process.env.DB_USER || 'ChadhaOuederni',
      password: process.env.DB_PASSWORD || 'Azerqsdf123*',
      database: process.env.DB_NAME || 'database-ouederni-chadha'
  });
